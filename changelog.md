...
### WIP: константное определение инструмента (`__comment__`)

**Цель:** перестать поддерживать в `__comment__` перечень возможностей,
который расходится с реальностью при каждой фиче.

**Решение:**
- `__comment__` в `backend-adapter.py` — однострочная **константа**:
  `backend router and endpoint adapter: [AN] Messages <-> [OI]-compatible
  backends for AI agents`. Прежний текст (5 строк перечисления: конвертация,
  стриминг, multi-backend, per-session, runtime-пул, WEBUI, Prometheus)
  удалён.
- Та же формулировка открывает вводный blockquote `README.md` (первой строкой,
  перед абзацем про версию) — единый смысл, продублированный осознанно.
- Политика зафиксирована в `CLAUDE.md`: буллет «Ключевые факты» описывает
  значение как константу и запрещает менять его при разработке/релизах без
  отдельного указания; в «Принципы» добавлен буллет «Определение инструмента —
  константа» со ссылкой на README.
- Ничем не парсится (проверено): `install.sh` берёт версию из баннера бинаря,
