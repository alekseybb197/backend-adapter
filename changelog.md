...
При старте адаптера, если `ADAPTER_DEBUG_LOGFILE` задан как директория (пер-сессийный режим), пустая/неустановленная сессия приводила к созданию пустого файла `session-*.unknown.log`. Начиная с v0.3.2, если `_last_log_session_id` не установлен (сессия ещё не началась), `_d()` возвращает без открытия файла — лишних нулефайлов не создаётся.
