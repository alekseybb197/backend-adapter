...
## v0.8.6 (WIP — тарифы моделей с колонкой Cost, пересмотр флагов логирования и WEBUI, корректное завершение по Ctrl-C/SIGTERM, Cost в live-поллинге, верхняя «Свернуть», тесты graceful shutdown и интеграционного прогона, README)
...
контролировать graceful shutdown тестом; (2) временный скрипт tmp/check_v086.py
проверяет реальное качество — перенести его в пул постоянных тестов; (3)
колонка Cost не считалась синхронно с обновлением счётчиков (висела «--»,
пересчёт лишь при добавлении новой модели); (4) у развёрнутой колонки моделей
ссылка «Свернуть» была только внизу.
...
- **постоянный интеграционный тест `tests/test_manual_check.py`** (маркер
  ...
  tmp/check_v086.py оставлен как привычная точечная проверка (синхронизирован);
...
временный интеграционный скрипт стал полноценным тестом; Cost на открытой
