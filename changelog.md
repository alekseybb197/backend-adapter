...
### 2026-09-06 Сброс строки таблицы моделей: POST /api/model-usage/reset + кнопка «Сбросить»

**Цель:** у таблицы использованных моделей не было способа убрать строку
модели (счётчики, результаты устаревших проб эндпоинтов) — кроме рестарта
адаптера с удалением `model-usage.yaml`. Добавить сброс строки одной модели
с веб-страницы и через JSON-API (полный сброс таблицы НЕ добавлялся).
...
- **эндпоинт `POST /api/model-usage/reset?model=<имя>`** — новый класс
  ...
  получает 200 `{"ok": true, "model": ...}` — сброшено, 404 `{"error": ...}`
  — строки нет, 400 `{"error": ...}` — нет query-параметра `model`;
...
**Следствия:** строка модели сбрасывается с веб-страницы и curl-ом без
рестарта и правки файла; сброс обновляет `model-usage.yaml` сразу
(`reset_model` → force-save); повторный сброс той же модели — 404;
поведение проксирования и учёта не меняется.
...
- **персистентность** — таблица сохраняется в `model-usage.yaml` в корне
  ...
  (сброс строки кнопкой + новое обращение — способ «освежить»);
...
- **сброс строки** — новая функция `model_usage.reset_model(model)`: удаляет
  строку из таблицы и из YAML-файла (полный сброс таблицы НЕ добавлялся);
