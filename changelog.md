...
## v0.9.2 (WIP — messages→messages passthrough: system-сообщения переносятся в начало; таблица активных сессий в WEBUI)

<!-- WIP: записи по мере согласованных коммитов группы v0.9.2. -->

### Таблица активных сессий агентов в WEBUI (v0.9.2)
...
- `session_registry.py` — новый модуль-лист DAG (импортирует только
  `config`, по образцу `model_usage.py`): in-memory таблица по `session_id`
  (agent/model/backend/route/last_seen/calls/errors + служебный `_ts`),
  публичный API `touch` / `set_route` / `record_error` / `sessions_snapshot`.
  Сортировка — по времени последнего обращения (новые сверху), при новом
  обращении старая сессия всплывает; эвикция до `ADAPTER_SESSIONS_TABLE`
  строк. **Без персистентности** (осознанное решение пользователя): таблица
  живёт только в памяти процесса, каждый запуск начинается заново;
- `server.py` — точки учёта: `touch` сразу после распознавания входного пути
  (до чтения тела — 400 «Invalid JSON»/«Missing model» тоже учтены; 404 на
  не-входной путь строку не создаёт); `set_route` после `routing.decide` (и
  для reject/disabled — видно, куда агент пытался); `record_error` — перехват
  в общих `_send_json`/`_send_raw` по финальному статусу ≥ 400 (покрывает все
  пути ошибок без правки ~20 точек вызова; no-op для незарегистрированной
  сессии, поэтому служебные ответы в счётчик не попадают). Имя агента —
  `User-Agent` до первого пробела;
- `webui_status.py` — секция «Sessions» после «Models in use» (8 колонок:
  ...
  (JSON для JS `sessions_poll`); live-обновление без перезагрузки — поллинг
  каждые ~5 с, строки сопоставляются по `data-session` (порядок меняется при
  всплытии сессий), при несовпадении состава — `location.reload()`;
- `config.py` — `ADAPTER_SESSIONS_TABLE` (дефолт 10; 0 — таблица отключена;
  вне runtime-пула `/config`);
- тесты: новый `tests/test_session_registry.py` (touch/эвикция/порядок/
  no-op/копия снимка), секция+эндпойнт в `test_webui_status.py`, учёт в
  `test_server.py` (`TestSessionAccounting`: route/agent/calls/errors, 404
