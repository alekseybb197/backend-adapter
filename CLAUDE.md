...
временного скрипта tmp/check_v086.py и — если он проверяет качество —
добавить его в пул постоянных тестов; (3) колонка Cost на странице не
считалась синхронно с обновлением счётчиков (висело «--», пересчёт лишь
при добавлении новой модели); (4) у развёрнутой колонки моделей ссылка
«Свернуть» была только внизу.
...
- **интеграционный прогон — постоянный тест `tests/test_manual_check.py`**
  ...
  контролем; tmp/check_v086.py остаётся как привычная точечная проверка;
...
интеграционными тестами (регрессия ловится в общем прогоне); ручной
интеграционный прогон стал постоянным тестом и переживает будущие
изменения; Cost на открытой странице меняется вместе со счётчиками без
перезагрузки; развёрнутую колонку моделей можно свернуть и сверху, и
снизу. Поведение проксирования не меняется. Ветка feature/v0.8.6 в WIP.
