...
Был ручным скриптом tmp/check_v086.py (запуск вручную, ad-hoc проверка
фич перед релизом); перенесён в пул постоянных тестов по замечанию «зачем
временный тест, если он проверяет качество — добавь в постоянные». Проверяет
то, что unit-тесты не видят: РЕАЛЬНЫЙ процесс backend-adapter.py с fake-
...
пропускается. Ручной скрипт tmp/check_v086.py оставлен как есть (привычная
точечная проверка; сюда дублируется синхронно при изменении контрактов).
...
# ---------------------------------------------------------------------------
# Хелперы (как в бывшем tmp/check_v086.py)
# ---------------------------------------------------------------------------
...
class TestManualAdapterProcess:
    ...
    завершения) — как в бывшем tmp/check_v086.py; ср. unit-покрытие в
    test_webui_status/test_shutdown. Намеренно один большой тест: спавн
    процесса дорог (стартовая проба бэкенда ~сек), дробить на части —
    множить время прогона."""
