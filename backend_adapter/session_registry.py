...
Что делает: фиксирует СООТВЕТСТВИЕ между клиентской сессией (id из первого
непустого заголовка списка ``ADAPTER_SESSION_HEADER``) и обработчиком её
трафика — именем агента (``User-Agent``), клиентской моделью, бэкендом и
выбранным правилом TARGET-маршрутизации, — дополняя его счётчиками
обращений/ошибок и временем последнего обращения. Состояние выводится таблицей
сессий на отдельной странице WEBUI "/sessions" (v0.9.5; см.
webui_sessions._sessions_rows_html).
...
``last_seen``. Поля-сопровождение (``input``, ``last_seen``, ``calls``,
``errors``) — не часть ключа, а описание уже состоявшегося кортежа: они не
создают новых строк и обновляются/читаются отдельно.
...
Строка таблицы (ключ — кортеж (session, agent, model, backend, route)):
    ...
    {"session": str,     # session_id как пришёл в заголовке
     ...
     "input": str,       # входной эндпоинт обращения: messages | completions |
                         #   responses (routing.input_path_to_format). Задаётся
                         #   АГЕНТОМ (выбор пути запроса) и потому для кортежа
                         #   константен — в отличие от route (куда адаптер
                         #   решил отправить). v0.9.5: колонка «Входной эндпоинт»
                         #   таблицы Sessions + ключ выбора TARGET-селекта строки.
...
Обнуление счётчиков строки — ``reset_counters`` (кнопка ⏪ рядом с 🗑, v0.9.5):
``calls``/``errors`` строки обнуляются, сама строка остаётся (в отличие от
удаления) и продолжает учитывать дальнейшие обращения с нуля.
"""
...
def register(
    ...
    *,
    ...
    что самодокументирует «недостроенный» кортеж. ``input`` — входной
    эндпоинт (v0.9.5): не часть ключа, а сопровождение строки; нужен колонке
    «Входной эндпоинт» и выбору TARGET-селекта строки в таблице Sessions."""
    try:
        if not session_id:
            return None
        limit = _limit()
...
def reset_counters(key: SessionKey) -> bool:
    """Обнулить счётчики ОДНОЙ строки-кортежа (кнопка ⏪ в таблице Sessions).

    Обнуляются только ``calls`` и ``errors`` строки; ``last_seen``/``_ts``
    (и, значит, позиция в сортировке) НЕ трогаются — строка остаётся на месте
    и продолжает учитывать дальнейшие обращения с нуля. Это принципиально:
    сессия может занимать НЕСКОЛЬКО строк (сменила модель/обработчик), и
    обнуление одной строки не должно сбрасывать счётчики соседних строк той
    же сессии (v0.9.5 — в отличие от пер-сессионных настроек логирования/
    роутинга, которые адресуются ``session_id``, а не кортежу).

    Возвращает True, если строка была и обнулена; False — строки нет (второй
    клик, кортеж вытеснен по лимиту или пересоздан заново). Не бросает
    исключений наружу — вызывающий (эндпойнт) решает, что ответить. Точка
    вызова — SessionResetEndpoint (POST /api/sessions/reset)."""
    try:
        with _TABLE_LOCK:
            row = _TABLE.get(key)
            if row is None:
