...
def stream_openai_to_anthropic(
    ...
    (включая пустые разделители и не-data-строки) — для учёта трафика
    (bytes_recv в model_usage). Список (а не возврат значения), чтобы не
    менять пару (stop_reason, usage), которую распаковывают вызывающие и
    тесты."""
