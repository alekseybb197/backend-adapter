...
Требует artifact_tree.py рядом (в той же директории) — импортируется
напрямую, а не дублирует логику генерации и не дёргает его через
subprocess. Формат логов настраивается ОДИН раз, здесь же (см.
artifact_tree.configure_logging()) — поэтому сообщения из этого файла и
сообщения из artifact_tree.generate() выходят в одном общем формате
"ГГГГ-ММ-ДД ЧЧ:ММ:СС [УРОВЕНЬ] сообщение", а не вперемешку разнобойными.
