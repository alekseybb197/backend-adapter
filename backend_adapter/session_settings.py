...
и это деление принципиально:
  ...
  - **TARGET-поля — ЖИВОЕ НАСЛЕДОВАНИЕ.** Три состояния:
    - НЕ ЗАДАНА (записи нет) — действует общая настройка приложения
      (``config.X``), включая последующие изменения через /config;
    - ``"inherit"`` (домен ``config.SESSION_TARGET_VALUES``) —
      переопределение существует, но трактуется как «взять общее». Нужно
      затем, чтобы в WEBUI поле можно было вернуть к общему значению, не
      удаляя запись вовсе: значение остаётся видимым и явным;
...
def effective(session_id: str, name: str) -> Any:
    ...
    * TARGET-поля — ЖИВОЕ наследование: переопределение (если задано и это не
      ``"inherit"``), иначе общая настройка приложения ``config.X`` (как
