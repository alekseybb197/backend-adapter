...
- **Где встречается:** заголовок `Authorization: Bearer <key>`; раскрытое значение env-переменной токена, на которую ссылается поле `key` YAML-конфига (пример — `docs/samples/sample.adapter.yaml`).
