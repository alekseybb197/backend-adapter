...
Отдельно в LOGPATH лежит **PID-файл** `adapter.pid` (пишется при любом запуске,
...
| Гейт записи | Только наличие лог-директории `ADAPTER_DEBUG_LOGPATH` (всегда непуста, дефолт `./tmp/logs`); **не** зависит от `ADAPTER_SESSIONS_TABLE` |
...
`404` выключенного входа (`ADAPTER_*_TARGET=none`), `400`/`502` reject
маршрута, `501` списка моделей.
...
| Гейт записи | Как у инцидента — только наличие `ADAPTER_DEBUG_LOGPATH`; не зависит от `ADAPTER_DEBUG_ENABLE`/`PARTS`/`TRIM` и от пер-сессионных Log/Parts |
...
### Пер-сессионные гейты логирования (v0.9.5, снимок — v0.9.8)

Флаги `ADAPTER_DEBUG` (файловая запись debug-логов) и `ADAPTER_DEBUG_PARTS`
(дампы частей) читаются **пер-сессионно**: `session_log.logging_enabled(session_id)`
и `session_log.parts_enabled(session_id)`.

**Файловая запись и дампы — СЕССИОННЫЕ функции (v0.9.8).** Запрос без
идентификатора сессии (`session_id` пуст или равен `unknown` — ни одного
заголовка из `ADAPTER_SESSION_HEADER`) файлов на диске **не создаёт**:
`logging_enabled`/`parts_enabled` возвращают `False`. Консольный канал это не
затрагивает — он безусловен (см. начало раздела).

**Две модели наследования (v0.9.8).** Значения Log/Parts для сессии берутся
**снимком** глобальных тумблеров в момент **образования сессии**
...
- глобальные `ADAPTER_DEBUG`/`ADAPTER_DEBUG_PARTS` **не включают и не
  выключают** функционал — они лишь шаблон для **новых** сессий: при
  глобальном Log=on новая сессия создаётся с Log=on, при off — с Log=off;
- смена глобального тумблера на `/config` **уже работающие сессии не трогает**
  (управление ими — на `/sessions`, выпадающие списки Log/Parts, ровно два
  положения `on`/`off`; состояния `inherit` у этих полей нет);
- «сбросить» Log/Parts у сессии = взять свежий снимок текущего общего тумблера
  (`set_config(..., clear=...)`), а не «вернуть живую связь».

Отличие от TARGET-полей: у них наследование **живое** (три состояния — не
задано / `inherit` / конкретное значение; см. `docs/routing.md` §2.4).

На безусловные каналы (`.err`, WARN-блоки, JSON-файлы проверок) пер-сессионные
гейты **не влияют** — они пишутся при любом `ADAPTER_DEBUG`/`PARTS`.
...
- Корень веб-сервера — директория `ADAPTER_DEBUG_LOGPATH` (по умолчанию
  ...
  PID-файл `adapter.pid` (v0.9.5 — пишется при любом запуске) и JSON-файлы
  результатов проверок `<бэкенд>.models.json` /
