...
- Проверка бэкендов (список моделей + дымовая проба эндпойнтов)
  запускается **только по кнопке «⟳ Проверить сейчас» (POST `/`)** и
  выполняется **в фоновом потоке**: `config.start_refresh` →
  `config.refresh_models()` (живой `GET /v1/models` каждого эндпойнта,
  таймаут 10 с на эндпоинт), кэш моделей адаптера пересобирается из живых
  ответов. Загрузка страницы (GET `/`) проверку **не запускает** — она
