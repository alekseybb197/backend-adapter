...
| Гейт записи | Как у инцидента — только наличие лог-папки `ADAPTER_DATA_ROOT/log`; не зависит от `ADAPTER_DEBUG_ENABLE`/`PARTS`/`TRIM` и от пер-сессионных Log/Parts |
...
независимо от `ADAPTER_DEBUG_ENABLE`, `ADAPTER_DEBUG_PARTS` и
...
**Поведение:**
- JSON/YAML файлы пишутся в ту же лог-папку, что и debug/trace-логи (`ADAPTER_DATA_ROOT/log`).
