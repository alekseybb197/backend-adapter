...
`session_id` — первый непустой заголовок из списка `ADAPTER_SESSION_HEADER`
(дефолт `X-Claude-Code-Session-Id,x-opencode-session`), иначе `unknown` —
...
| Block | Описание |
|---|---|
| `[REQ]` | HTTP metadata: метод, путь, session_id (первый непустой заголовок из списка `ADAPTER_SESSION_HEADER`, иначе `unknown`) |
