...
выше). С v0.9.5 в тот же файл пишутся **ЛЮБЫЕ ошибки, учтённые в таблице
Sessions** (`server._note_session_error`) — включая те, что до бэкенда не
дошли: `400` валидации тела (Invalid JSON / нет `model` / strict-модель),
`404` выключенного входа (`ADAPTER_*_TARGET=none`), `400`/`502` reject
маршрута, `501` списка моделей. Инвариант: **сколько ошибок учтено в
таблице — столько ERROR-блоков в `.err`**; счётчик «Ошибок» строки `/sessions`
служит прямой ссылкой на этот файл.
...
### Пер-сессионные гейты логирования (v0.9.5)
...
и `session_log.parts_enabled(session_id)` возвращают переопределение сессии
(`session_settings.effective`) либо, если его нет, общую настройку приложения.
Пустой `session_id` → общая настройка. Переопределения задаются в таблице
`/sessions` (выпадающие списки Log / Parts) либо `POST /api/sessions/settings`;
живут в памяти процесса, переживают вытеснение строки из таблицы, но не
перезапуск адаптера.
