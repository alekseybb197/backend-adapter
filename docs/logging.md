...
- Корень веб-сервера — `ADAPTER_DATA_ROOT` (по умолчанию `./tmp/adapter`).
  ...
  `/session` пуста.
- Дерево собирается **фоново, вместе с записью частей** (v0.9.10): пока
  `ADAPTER_DEBUG_ENABLE=1`, каждая успешно записанная часть помечает свою
  `<сессия>.parts/` как «грязную» (`artifact_refresh.notify`), а отдельный
  daemon-поток раз в секунду перерисовывает директорию, когда после
  **последней** записи прошёл тихий период `ADAPTER_ARTIFACT_DEBOUNCE`
  секунд (по умолчанию 20) — серия частей одного хода даёт одну перерисовку.
  Рендер идёт в фоне, поэтому **запрос прокси его никогда не ждёт**.
  Повторные генерации — **инкрементальные**: `generate()` ведёт чекпоинт
  `artefacts/.build_state.json` и обрабатывает только новые `*.parts` файлы
  сессии (номер части больше чекпоинтного); разметка (Finish/Superseded/Title,
  ответы на запросы, границы страниц) пересчитывается по полному состоянию.
  Чекпойнт битый или отсутствует — холодный полный пересбор с warning.
  Обновите страницу браузера, чтобы увидеть актуальное состояние (активная
  вкладка при этом сохранится — см. ниже).
- Страховка и ручной путь: если `artefacts/tree.html` отсутствует или устарел
  (адаптер перезапустился, воркер ещё ничего не видел), `artifact_tree.generate()`
  пересоздаёт дерево при заходе на `/session`; тот же публичный API доступен
  из CLI (`python -m backend_adapter.artifact_tree`).
