...
Официальная инструкция: <https://code.anthropic.com/docs/en/setup>
(страница **Advanced setup** — требования, способы установки, обновление и
удаление). Быстрый обзор первого запуска — отдельная страница
<https://code.anthropic.com/docs/en/quickstart>.

**Требования.** macOS 13.0+; Windows 10 1809+ (или Windows Server 2019+);
Ubuntu 20.04+; Debian 10+; Alpine Linux 3.19+. 4 ГБ ОЗУ, процессор x64 или
ARM64, оболочка bash/zsh/PowerShell/CMD, доступ в интернет.

**Установка.** Рекомендуемый способ — **native-инсталлятор** (macOS/Linux/WSL,
а также PowerShell и CMD в Windows): он сам обновляется в фоне и не требует
менеджера пакетов. Готовые команды для каждой платформы — на странице
Advanced setup по ссылке выше. Альтернативы — менеджеры пакетов:

```bash
# Homebrew (macOS/Linux) — стабильный канал;
# для свежих релизов — cask claude-code@latest
brew install --cask claude-code

# WinGet (Windows)
winget install Anthropic.ClaudeCode

# npm (Node.js 22 или новее)
npm install -g @anthropic-ai/claude-code
```

> **Про URL native-инсталлятора.** В официальной документации URL загрузки
> вырезаны (приходят пустыми), поэтому здесь они не воспроизводятся —
> возьмите точную команду для своей платформы со страницы Advanced setup.
> Команды менеджеров пакетов приведены дословно.

**Проверка установки:**

```bash
claude --version     # печатает номер версии, например 2.1.211 ([CC])
claude doctor        # подробная диагностика установки и настроек
```

**Вход.** Запустите `claude` и следуйте подсказкам браузера; смена аккаунта —
команда `/login` в сессии. Для работы через локальный адаптер аутентификация
Anthropic не нужна: важно лишь **оставить `ANTHROPIC_API_KEY` пустым**
(раздел 2).
...
Список слэш-команд — `/help`. Обновление: native-установка обновляется
автоматически в фоне (вручную — `claude update`), установки через Homebrew,
WinGet и npm по умолчанию требуют ручного обновления
(`brew upgrade claude-code`, `winget upgrade Anthropic.ClaudeCode`,
`npm install -g @anthropic-ai/claude-code@latest`).

---

## 2. Как [CC] подключается к адаптеру
...
## 3. Файлы настроек: расположение и формат
...
- в репозиторий backend-adapter каталог `.claude/` **не коммитится**
  ...
  `docs/claude_code/` и копируются вручную (раздел 6). Для **других**
...
## 4. Пользовательский профиль — `~/.claude/settings.json`
...
| `env` | объект | Переменные окружения для каждой сессии и её подпроцессов (см. 4.2) |
| `statusLine` | объект | Рисовать свою строку состояния (см. раздел 7) |
...
| Переменная | Значение | Назначение |
|---|---|---|
| `ANTHROPIC_BASE_URL` | `http://localhost:9999` | Адрес адаптера (раздел 2) |
...
весь набор ролей и модель субагентов** — разбор рисков в разделе 4.2.
...
### 4.2 Выбор моделей и размер контекста — в чём отличие от QwenCode
...
## 5. Проектные настройки — `<project>/.claude/settings.json`
...
### 5.1 Комментарии по ключам

| Ключ | Значение | Назначение |
|---|---|---|
| `alwaysThinkingEnabled` | `true` | Для этого проекта расширенное мышление включено по умолчанию — в отличие от пользовательского профиля, где оно выключено (`false`, раздел 4). Проектные значения при слиянии имеют приоритет |
...
## 6. Установка типовых файлов

```bash
# 1. Пользовательский профиль
cp docs/claude_code/global.settings.json ~/.claude/settings.json
#    поправьте в нём имена моделей (раздел 4.2) — файл должен остаться
...
(раздел 4), он же указывается в `statusLine.padding` — отступ строки от
края (в примере `0`).
