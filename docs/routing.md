...
> **Отличие от Log (v0.9.8).** **Живым** наследованием общей настройки
> (после снятия состояния `inherit` — v0.9.9) обладают только TARGET-поля.
> Пер-сессионный Log устроен иначе: его значение берётся **снимком** общего
> тумблера при образовании сессии и дальше от `config` не зависит
